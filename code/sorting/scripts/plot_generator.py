...
COLORES = {
    'sort': '#ff598f',         # Rosa brillante
    'mergesort': '#00d2ff',    # Cian
    'quicksort': '#b05cff',    # Violeta
    'patiencesort': '#ffaa00'  # Naranja
}

def generar_graficos():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    csv_path = os.path.join(base_dir, "../data/measurements/sorting_measurements.csv")
    ...
    for t in df["tipo"].unique():
        for d in df["dominio"].unique():
            ...
            ax.set_xlabel("Tamaño del arreglo (N)", labelpad=15)
            ax.set_ylabel("Tiempo promedio (ms)", labelpad=15)
            ax.set_title(f"Tiempo vs N ({t} - {d})", pad=25)

            ax.legend(title="Algoritmo", frameon=False, loc="upper left")
            plt.tight_layout()

            nombre = f"sorting_{t}_{d}.png"
